...
import { getMoviesByKeyword, getTVByKeyword } from "../api/search";
...
export default function KeywordDetail() {
  ...
  const [keywordName, setKeywordName] = useState<string>((location.state as any)?.name ?? "");

  const loadMovies = useCallback(async (page: number, append = false) => {
    if (!keywordId) return;
    append ? setLoadingMore(true) : setLoading(true);
    try {
      const res = await getMoviesByKeyword(keywordId, page);
      const data = res.data;
  ...
  const loadTV = useCallback(async (page: number, append = false) => {
    ...
    append ? setLoadingMore(true) : setLoading(true);
    try {
      const res = await getTVByKeyword(keywordId, page);
      const data = res.data;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      {/* Header */}
      <div className="mb-8">
        <p className="text-gray-500 text-sm mb-1">Keyword</p>
        <h1 className="text-white text-2xl font-bold">
          # {keywordName || `Keyword #${keywordId}`}
        </h1>
