...
export default function TVPhotos() {
  const { id } = useParams();
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      ...
      {lightbox && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4" onClick={() => setLightbox(null)}>
          ...
          />
          <button onClick={() => setLightbox(null)} className="absolute top-4 right-4 text-white text-2xl hover:text-gray-300 bg-black/50 w-10 h-10 rounded-full flex items-center justify-center">✕</button>
          <a
      ...
      <div className="flex gap-2">
        {(["backdrops", "posters"] as Tab[]).map((tab) => (
          <button
            key={tab}
          ...
          >
            ...
            <span className="ml-1.5 text-xs opacity-70">{tab === "backdrops" ? backdrops.length : posters.length}</span>
          </button>
        ))}
      </div>
      ...
      {current.length === 0 ? (
        ...
        <div className={activeTab === "backdrops" ? "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3" : "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3"}>
          {current.map((img, i) => (
            <button
              key={i}
            ...
            >
              <img
                ...
                alt={`${activeTab} ${i + 1}`}
                className={`w-full object-cover group-hover:opacity-80 transition ${activeTab === "backdrops" ? "aspect-video" : "aspect-[2/3]"}`}
              />
